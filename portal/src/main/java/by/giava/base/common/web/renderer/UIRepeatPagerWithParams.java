...
public class UIRepeatPagerWithParams extends Renderer {
	...
	public void encodeBegin(FacesContext context, UIComponent component)
		...
		writeLinks(writer, component, styleClass, selectedStyleClass,
				currentpage, first, startPage, endPage, pages, itemcount,
				pagesize, tipo, filtro);
	}

	private void writeLinks(ResponseWriter writer, UIComponent component,
			String styleClass, String selectedStyleClass, int currentPage,
			int first, int startPage, int endPage, int pages, int itemcount,
			int pagesize, String tipo, String filtro) throws IOException {

		String[] paramPageFirst = new String[] { "currentpage", "0" };
		String[] paramPageLast = new String[] { "currentpage", "" + endPage };
		String[] paramPagePrevious = new String[] { "currentpage",
				"" + (currentPage - 1) };
		String[] paramPageNext = new String[] { "currentpage",
				"" + (currentPage + 1) };
		String[] paramTipo = new String[] { "tipo", tipo };
		String[] paramFiltro = new String[] { "filtro", filtro };

		if (startPage > 0) {
			writeLink(writer, component, "<<", styleClass, new String[][] {
					paramPageFirst, paramTipo, paramFiltro });
		}

		if (currentPage > 0)
			writeLink(writer, component, "<", styleClass, new String[][] {
					paramPagePrevious, paramTipo, paramFiltro });

		for (int i = startPage; i < endPage; i++) {
			writeLink(writer, component, "" + (i + 1),
					i == currentPage ? selectedStyleClass : styleClass,
					new String[][] { new String[] { "currentpage", "" + i },
							paramTipo, paramFiltro });
		}

		if (first < itemcount - pagesize)
			writeLink(writer, component, ">", styleClass, new String[][] {
					paramPageNext, paramTipo, paramFiltro });

		if (endPage < pages)
			writeLink(writer, component, ">>", styleClass, new String[][] {
					paramPageLast, paramTipo, paramFiltro });

	}

	private void writeLink(ResponseWriter writer, UIComponent component,
			String value, String styleClass, String[][] params)
			throws IOException {
