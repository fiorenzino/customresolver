...
public class PersistenceContextExtension implements Extension {
	...
	void processProducer(@Observes ProcessProducer<?, EntityManager> pp,
		...
		if (pp.getAnnotatedMember().isAnnotationPresent(
			...
			if (emfBean == null) {
				AnnotatedField<?> field = (AnnotatedField<?>) pp
						.getAnnotatedMember();
				final String unitName = field.getAnnotation(
						PersistenceContext.class).unitName();
				final Class<?> module = field.getJavaMember()
						.getDeclaringClass();
				final Set<Annotation> qualifiers = new HashSet<Annotation>();
				for (Annotation ann : field.getAnnotations()) {
					Class<? extends Annotation> annotationType = ann
							.annotationType();
				...
				final boolean alternative = field
						.isAnnotationPresent(Alternative.class);
