 ...
 * <p>
 * 
 * Adds a varStatus feature to the UIRepeat tag delivered with facelets. The
 * var-status contains the following fields:
 * 
 * index ... the current index (zero based) count ... the iteration count
 * (number of iteration, which means: the size of the collection) first ...
 * true, if the current iteration is the first (true, if index==0) last ...
 * true, if the current iteration is the last one (true, if index==count-1)
 * 
 * If you need an iteration without a collection, you can use from and to
 * instead of value!
 * 
 * @author $Author$
