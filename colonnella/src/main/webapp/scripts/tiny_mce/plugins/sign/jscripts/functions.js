...
function ts_upload_image_path() {
  ...
  to_path = "/my_images.js";
  return to_path;
}
function ts_get_images_path(){
  // business_alias = window.parent.location.pathname.split("/")[1];
  to_path ="/my_images";
  return to_path;
}
...
function preinit() {
	...
	var url = tinyMCE.activeEditor.getParam("external_image_list_url");
	if (url != null) {
		// Fix relative
...
function convertURL(url, node, on_save) {
	//return eval("window.opener." + tinyMCE.settings['urlconverter_callback'] + "(url, node, on_save);");
	return "gestione-convocazione.jsf";
}
...
function getImageSrc(str) {
	...
	if ((pos = str.indexOf('this.src=')) != -1) {
		...
		if (tinyMCE.activeEditor.getParam('convert_urls'))
...
function init() {
	...
	if (action == "update") {
		...
		if (onmouseoversrc != "" && tinyMCE.activeEditor.getParam('convert_urls'))
			onmouseoversrc = convertURL(onmouseoversrc, elm, true);

		if (onmouseoutsrc != "" && tinyMCE.activeEditor.getParam('convert_urls'))
	...
	if (tinyMCE.activeEditor.getParam("advimage_constrain_proportions", true))
...
function insertAction() {
	...
	if (tinyMCE.activeEditor.getParam("accessibility_warnings")) {
...
function updateStyle() {
	...
	if (tinyMCE.activeEditor.getParam('inline_styles', false)) {
...
function showPreviewImage(src, start) {
	...
	selectByValue(formElement(), 'imagelistsrc', src);

	var elm = document.getElementById('prev');
	var src = src == "" ? src : tinyMCE.convertRelativeToAbsoluteURL(tinyMCE.settings['base_href'], src);

	if (!start && tinyMCE.activeEditor.getParam("advimage_update_dimensions_onchange", true))
		resetImageData();
